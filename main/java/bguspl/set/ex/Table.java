...
public class Table {
	...
	protected ArrayList<Integer>[] tokensToSlot;
	...
	public Table(Env env, Integer[] slotToCard, Integer[] cardToSlot) {
		...
		this.tokensToSlot = new ArrayList[slotToCard.length];
		for (int i = 0; i < slotToCard.length; i++) {
			tokensToSlot[i] = new ArrayList<Integer>();// inside the list we place the player id that place the token
		}
	}
	...
	public void placeCard(int card, int slot) {
		synchronized (tokensToSlot[slot]) {
			try {
	...
	public void removeCard(int slot) {
		synchronized (tokensToSlot[slot]) {
			...
			// TODO implement
			if (slotToCard[slot] != null) {// do changes on dealer
	...
	public void placeToken(int player, int slot) {
		// TODO implement
		synchronized (tokensToSlot[slot]) {
			if (slotToCard[slot] != null && !tokensToSlot[slot].contains(player)) {
	...
	public boolean removeToken(int player, int slot) {
		...
		synchronized (tokensToSlot[slot]) {
			if (slotToCard[slot] != null && tokensToSlot[slot].contains(player)) {
				...
				tokensToSlot[slot].remove(tokensToSlot[slot].indexOf(player));
				return true;
			}
			return false;
		}
